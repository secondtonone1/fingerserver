...
MD5Hash::MD5Hash(const char* sum)
{
	initial();
	mCommitted = true;
...
void MD5Hash::update(void)
{
	...
	for(i = 0; i < 16; ++i)
    {
		x[i] = (unsigned long)(mBuf[i * 4]) |
			   (unsigned long)(mBuf[i * 4 + 1] << 8) |
			   (unsigned long)(mBuf[i * 4 + 2] << 16) |
			   (unsigned long)(mBuf[i * 4 + 3] << 24);
    }
...
void MD5Hash::commit(void)
{
	...
	mCount[0] += (unsigned long)(mBPos << 3);
	if(mCount[0] < (unsigned long)(mBPos << 3))
    {
		++mCount[1];
    }
	...
	for(i = 0; i < 2; ++i)
	{
		cbuf[i * 4] = (unsigned char)mCount[i] & 0xff;
		cbuf[i * 4 + 1] = (unsigned char)((mCount[i] >> 8) & 0xff);
		cbuf[i * 4 + 2] = (unsigned char)((mCount[i] >> 16) & 0xff);
		cbuf[i * 4 + 3] = (unsigned char)((mCount[i] >> 24) & 0xff);
	}

	i = (unsigned) ((mCount[0] >> 3) & 0x3f);
	len = (i < 56) ? (56 - i) : (120 - i);
	if(len)
    {
	...
	for(i = 0; i < 4; ++i)
	{
		mMd5[i * 4] = (unsigned char)mState[i] & 0xff;
		mMd5[i * 4 + 1] = (unsigned char)((mState[i] >> 8) & 0xff);
		mMd5[i * 4 + 2] = (unsigned char)((mState[i] >> 16) & 0xff);
		mMd5[i * 4 + 3] = (unsigned char)((mState[i] >> 24) & 0xff);
	}
	initial();
	mCommitted = true;
}

bool MD5Hash::isCommitted() const
{
	//return (mCommitted && !mBPos);
	return mCommitted;
}

void MD5Hash::put(const char* buf, unsigned int size)
{
	while(size--) 
    {
		mBuf[mBPos++] = *(buf++);
...
MD5Hash::operator const unsigned char*()
{
	commit();
	return mMd5;
}
...
{
	commit();
	return toString(mMd5);
}

String MD5Hash::toTempString()
{
	char buf[512];
	//save state
	memcpy(buf, this, sizeof(MD5Hash));
	String result = toString();
	//restore state
	memcpy(this, buf, sizeof(MD5Hash));
	return result;
}

String MD5Hash::toString(const unsigned char* md5)
{
	char buf[36];
...
bool MD5Hash::operator==(const MD5Hash& sum)
{
	commit();
	//if(isCommitted() && sum.isCommitted()) {
	return (memcmp(mMd5, sum.mMd5, 16) == 0);
	//}
}
