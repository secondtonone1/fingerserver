...
{
    return _vsnprintf_s(_DstBuf, _SizeInBytes, _SizeInBytes - 1, _Format, _ArgList);
}
...
void 
Logger::dispatchThreadMsg(ThreadMsg* ev)
{
    if (mOutputFile)
    {
        int nBytes = 0;
        while (1)
        {
            StreamBuffer::Node* node = ev->mBody.popNode();
            if (node == NULL)
            {
                break;
            }
