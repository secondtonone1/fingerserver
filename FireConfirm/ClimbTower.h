...
namespace Lynx
{
	...
	struct EventList
	{
		EventList(): startTime(0),HPStartTime(0),towerHight(0),roadType(1),lastJumpFloor(0),latestJumpFloor(0),latestCrossRoadFloor(0),reliveTimes(0){}
		UInt32 startTime;
		UInt32 HPStartTime;
		UInt32 towerHight;
		UInt32 roadType;//1路一，2 路二  大于1为路2
		UInt32 reliveTimes;
...
// 					leaf.append(it->mValue.tID);					
	...
	struct TowerEndResp
	{
		...
		std::string convertDataToJson()
		{
			...
			for(List<UInt32>::Iter * iter = awardIndexList.begin(); iter != NULL; iter = awardIndexList.next(iter))
			{
							
				root["awardIndexList"].append(iter->mValue);
			}
			Json::FastWriter writer;  
			std::string strWrite = writer.write(root);
			return strWrite;
...
// 		static void onTowerBuffReq(const  ConnId& connId,TowerBuffReq & req);
		...
		//事件完成结算
		UInt32 addEventValues(Guid playerID,InTowerChoiseReq & req,InTowerChoiseResp &resp,UInt32 road);

		//加物品
		void dealTowerGoods(Guid playerID,List<Goods> goodsList);

		bool addTowerData(Guid playerID,List<Goods> goods,UInt32 dataType,int num);
